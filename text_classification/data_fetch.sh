...
# 2020-12-11 RJR for UIUC CS 410 project, I only use Yelp Review polarity
  #ag_news
  #sogou_news
  #dbpedia
  #yelp_review_full
...
# 2020-12-11 RJR for UIUC CS 410 project, I manually downloaded the CSV from Google Drive
# and used this script only to normalize the text with the function above
...
do
  ...
  then
    ...
    cat "${DATADIR}/${DATASET[i]}_train.csv" | normalize_text > "${DATADIR}/${DATASET[i]}.train"
    cat "${DATADIR}/${DATASET[i]}_test.csv" | normalize_text > "${DATADIR}/${DATASET[i]}.test"
  fi
done
